...
contract PropertyRental is Ownable, ReentrancyGuard, Pausable {
    ...
    // user
    User[] public users;
    mapping(address => User) public usersMap; // bir kiracıyı adresine göre getirmemizi sağlayan kiracı
    ...
    function createProperty(
        ...
        bytes32 _id =  keccak256(abi.encode(_name, address(msg.sender), _createdAt));
    ...
    function putPropertyInTheAd(
        ...
        require(_currentProperty.holderAddress == address(msg.sender), "Only owner!");
    ...
    function requestToBecomeTenant(bytes32 _propertyId) external nonReentrant returns(bool){
        Property memory _currentProperty = propertiesMap[_propertyId];
        User memory _currentUser = usersMap[msg.sender];
        require(_currentProperty.holderAddress != address(msg.sender), "You are the owner!");
    ...
    function startRentalContract(bytes32 _propertyId, Property memory _propertyType, uint256 _startDate, uint256 _endDate, address _tenantAddress, uint256 _createdAt) external nonReentrant returns(bool){
        Property storage _currentProperty = propertiesMap[_propertyId];
        require(msg.sender == _currentProperty.holderAddress);
        _currentProperty.tenantAddress = _tenantAddress;
        bytes32 _id =  keccak256(abi.encode(_propertyId, address(msg.sender), _tenantAddress, _createdAt));
    ...
    function finishRentalContract(bytes32 _rentalContractId) external nonReentrant returns(bool) {
        RentalContract storage _rentalContract = rentalContractsMap[_rentalContractId];
    ...
    function makeComplaint(bytes32 _propertyId, string memory _reason, uint256 _createdAt) external nonReentrant returns(bool){
        ...
        complaintsMap[_id] = complaint// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;
...
contract PropertyRental is Ownable, ReentrancyGuard, Pausable {
    ...
    // sözleşmeyi sonlandır (erken tarih kontrolü)
    function finishRentalContract(bytes32 _rentalContractId, uint256 _date) external nonReentrant {
        RentalContract storage _rentalContract = rentalContractsMap[_rentalContractId];
